...
import { createCharacter } from "@dracor/database";
...
const WEAPON_EXTRA: Record<string, {
  playstyle: string; bestWith: string; scaling: string; category: string;
}> = {
  dagger: {
    playstyle: "Lightning-fast assassin. Low damage per hit but extreme attack speed. Crit-focused burst combos reward precision.",
    bestWith: "Storm (dodge + speed) or Ember (crit burn stacks)",
    scaling: "Agility + Focus",
    category: "Melee",
  },
  longsword: {
    playstyle: "Noble warrior. Strong per-hit damage with measured pace. Rune channel amplifies Dragon Memory on heavy strikes.",
    bestWith: "Ember (sustained burn) or Stone (frontline durability)",
    scaling: "Strength + Agility",
    category: "Melee",
  },
  arming_sword: {
    playstyle: "Frontline all-rounder. Balanced damage and speed with parry mechanic. The most forgiving starter weapon.",
    bestWith: "Stone (tankline) or Ember (steady DPS)",
    scaling: "Strength + Vitality",
    category: "Melee",
  },
  greatsword: {
    playstyle: "Devastating power strikes. Very slow but massive damage and cleave. Staggers enemies and punishes groups.",
    bestWith: "Stone (endurance) or Ember (ignite on cleave)",
    scaling: "Strength + Vitality",
    category: "Melee",
  },
  bow: {
    playstyle: "Ranged precision. High damage ceiling but punishes missed shots. Positioning and kiting are everything.",
    bestWith: "Storm (rapid fire) or Ember (burning arrows)",
    scaling: "Agility + Focus",
    category: "Ranged",
  },
  arrows: {
    playstyle: "Rapid ranged volleys. Lower per-hit damage but fast attack speed. Multi-target pressure through volume of fire.",
    bestWith: "Storm (chain lightning arrows) or Ember (area burn)",
    scaling: "Agility + Focus",
    category: "Ranged",
  },
  axe: {
    playstyle: "Aggressive berserker. Heavy one-handed strikes with bonus damage on wounded targets. Relentless offensive pressure.",
    bestWith: "Ember (rage amplification) or Storm (frenzy speed)",
    scaling: "Strength + Agility",
    category: "Melee",
  },
  hammer: {
    playstyle: "Guardian crusher. Slow but staggers enemies and reduces their defenses. Tied to Stone Memory lore.",
    bestWith: "Stone (shatter combos) or Ember (molten impact)",
    scaling: "Strength + Vitality",
    category: "Melee",
  },
  spear: {
    playstyle: "Skirmisher with reach. Keeps enemies at distance with precise thrusts. Hit-and-run playstyle rewards patience.",
    bestWith: "Storm (dash-thrust combos) or Stone (hold the line)",
    scaling: "Agility + Focus",
    category: "Melee",
  },
  staff: {
    playstyle: "Channeled caster. Slow but devastating burst. Staff orb amplifies Dragon Memory element into area devastation.",
...
const WEAPON_LORE: Record<string, string> = {
  dagger: "Speed is the truest armor. A dagger in skilled hands ends fights before they begin.",
  longsword: "Steel speaks a noble language: cut, sweep, riposte. The long sword is honor made edge.",
  arming_sword: "The frontier's most trusted companion. Balanced, reliable, and deadly in any hand.",
  greatsword: "When the world refuses to bend, bring a blade that makes it kneel.",
  bow: "Distance is safety. Distance is power. The bow turns patience into lethality.",
  arrows: "A quiver full of dragon-tooth tips is a conversation ender at any range.",
  axe: "The axe does not negotiate. It splits what stands before it — shield, armor, or bone.",
  hammer: "The Stone remembers: patience outlasts fury. The hammer is that patience, given weight.",
  spear: "Reach defines the battlefield. The spear decides where the enemy may and may not stand.",
  staff: "Wood and will. The staff channels what the body cannot contain — raw elemental devastation.",
};
...
function WeaponSelectionStep({
  ...
  const selectedExtra = selectedWeapon ? WEAPON_EXTRA[selectedWeapon] : null;
  const selectedItem = STARTER_WEAPONS.find((w) => w.metadata?.weaponClass === selectedWeapon);
    ...
    <div className="flex h-full flex-col lg:flex-row">
      ...
      <div className="wizard-panel flex flex-col gap-1 overflow-y-auto border-r border-stone-800/30 p-3 lg:w-72">
        {STARTER_WEAPONS.map((w) => {
          ...
          const isActive = selectedWeapon === weaponId;
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2.5 pl-1.5">
                <span className="text-lg shrink-0">{WEAPON_ICONS[weaponId] || "⚔️"}</span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className={`text-[13px] font-bold truncate ${isActive ? "text-ember-400" : "text-stone-300 group-hover:text-stone-100"}`}>
                      {w.name.replace("Dracor ", "")}
                    </h3>
                    <span className={`shrink-0 rounded px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider ${
                      extra?.category === "Ranged" ? "bg-sky-500/15 text-sky-400"
                        : extra?.category === "Caster" ? "bg-violet-500/15 text-violet-400"
                        : "bg-red-500/15 text-red-400"
                    }`}>
                      {extra?.category || "Melee"}
                    </span>
                  </div>
                  <p className="mt-0.5 text-[10px] text-stone-600 truncate">{extra?.scaling}</p>
                </div>
              </div>
            </button>
          );
      ...
      <div className="relative flex-1 min-h-[300px]">
        ...
        {selectedWeapon && (
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-stone-950 via-stone-950/50 to-transparent p-8 pt-20">
            ...
            <p className="mt-2 mx-auto max-w-md text-[13px] italic leading-relaxed text-stone-400 animate-fade-in">
              &ldquo;{WEAPON_LORE[selectedWeapon]}&rdquo;
            </p>
          </div>
        )}
        {!selectedWeapon && (
          <div className="flex h-full items-center justify-center bg-stone-950">
            <div className="text-center">
              <p className="font-display text-lg text-stone-700 tracking-wider">Claim a Weapon</p>
            </div>
          </div>
        )}
      </div>
      ...
      {selectedWeapon && selectedItem && selectedExtra && (
        <div className="wizard-panel flex flex-col gap-4 overflow-y-auto border-l border-stone-800/30 p-4 lg:w-[400px] animate-fade-in" key={selectedWeapon}>
          {/* Category badge */}
          <div className="flex items-center gap-3">
            <span className={`rounded-md px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest ${
              selectedExtra.category === "Ranged" ? "bg-sky-500/15 text-sky-400"
                : selectedExtra.category === "Caster" ? "bg-violet-500/15 text-violet-400"
                : "bg-red-500/15 text-red-400"
            }`}>
              {selectedExtra.category}
            </span>
            <span className="text-[10px] uppercase tracking-wider text-stone-600">
              {selectedExtra.scaling}
            </span>
          </div>

          {/* Description */}
          <div className="rounded-lg bg-white/[0.02] border border-white/[0.04] px-4 py-3">
            <p className="text-[12px] font-medium leading-relaxed text-stone-300">{selectedItem.description}</p>
          </div>

          {/* Stats */}
          <div className="rounded-lg border border-white/[0.04] bg-white/[0.015] p-4">
            <h3 className="mb-3 text-[10px] font-bold uppercase tracking-[0.2em] text-stone-600">Combat Stats</h3>
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded bg-white/[0.03] px-3 py-2.5 text-center">
                ...
                <p className="text-[16px] font-black tabular-nums text-stone-200">{selectedItem.damageMin}&ndash;{selectedItem.damageMax}</p>
              </div>
              <div className="rounded bg-white/[0.03] px-3 py-2.5 text-center">
                <p className="text-[8px] font-bold uppercase tracking-wider text-stone-600">Speed</p>
                <p className="text-[16px] font-black tabular-nums text-stone-200">{(selectedItem.metadata?.attackSpeed as number)?.toFixed(1)}x</p>
              </div>
              <div className="rounded bg-white/[0.03] px-3 py-2.5 text-center">
                <p className="text-[8px] font-bold uppercase tracking-wider text-stone-600">DPS</p>
                <p className="text-[16px] font-black tabular-nums text-ember-400">
                  {(((selectedItem.damageMin || 0) + (selectedItem.damageMax || 0)) / 2 * (selectedItem.metadata?.attackSpeed as number || 1)).toFixed(1)}
                </p>
              </div>
              <div className="rounded bg-white/[0.03] px-3 py-2.5 text-center">
                <p className="text-[8px] font-bold uppercase tracking-wider text-stone-600">Scales</p>
                <p className="text-[12px] font-bold text-stone-300">{selectedExtra.scaling.split(" + ").map(s => s.slice(0,3)).join(" + ")}</p>
              </div>
            </div>
          </div>
          ...
          {/* Playstyle */}
          <div>
            <h3 className="mb-2 text-[10px] font-bold uppercase tracking-[0.2em] text-stone-600">Playstyle</h3>
            <div className="rounded-lg border border-white/[0.04] bg-white/[0.015] p-3">
              <p className="text-[11px] leading-relaxed text-stone-400">{selectedExtra.playstyle}</p>
            </div>
          </div>

          {/* Synergy */}
          <div className="rounded-lg border border-white/[0.04] bg-white/[0.01] p-3">
            <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-stone-600 mb-1">Best Paired With</h4>
            <p className="text-[11px] leading-relaxed text-stone-400">{selectedExtra.bestWith}</p>
          </div>
        </div>
      )}
    </div>
...
const VALID_RACES: RaceId[] = ["dracor", "ironborn", "sylvhari", "ashwalker", "voidtouched", "bloodfane", "stoneguard", "grukhar", "skrix"];

function CharacterCreationWizard() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const initialRace = (() => {
    const q = searchParams.get("race");
    return q && (VALID_RACES as string[]).includes(q) ? (q as RaceId) : "dracor";
  })();
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (pgError.code === "23505") {
        ...
        setError(pgError.message || "Something went wrong. Try again.");
      }
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-stone-950">
      ...
      <div className="relative z-10 flex items-center justify-between border-b border-stone-800/30 bg-stone-950/80 px-5 py-3 backdrop-blur-xl">
        <button
          onClick={() => router.push("/account/characters")}
